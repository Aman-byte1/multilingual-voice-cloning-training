#!/usr/bin/env python3
"""
Cross-Lingual Voice Cloning Evaluation Pipeline — VoxCPM2
==========================================================
Zero-shot VoxCPM2 evaluation on ymoslem/acl-6060.
...
def main():
    parser = argparse.ArgumentParser(
        description="Cross-Lingual Voice Cloning Evaluation — VoxCPM2")
    ...
    # PHASE 2: Load VoxCPM2
    print(f"\n🔧 Phase 2: Loading VoxCPM2 model")
    from voxcpm import VoxCPM
    import soundfile as sf
    
    # load_denoiser=False for standard generation
    model = VoxCPM.from_pretrained(args.model_name, load_denoiser=False)
    print("   VoxCPM2 loaded ✓")

    VOX_SR = model.tts_model.sample_rate
    ...
    for entry in tqdm(manifest, desc="Generating"):
        ...
        elapsed = t1 - t0
        audio_dur = len(wav) / VOX_SR if VOX_SR > 0 else 0
    ...
    print("\n" + "=" * 62)
    print("  VoxCPM2 EVALUATION COMPLETE")
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["idx", "WER", "CER", "Similarity", "InferenceS", "AudioDurS", "RTF"])
        writer.writeheader()
