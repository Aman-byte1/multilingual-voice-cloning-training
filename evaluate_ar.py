...
def main():
    ...
    text_file = "blind_test/text/arabic.txt"
    report_file = "eval_results_ar_sample.json"
    ...
    print(f"🔍 Sampling {len(eval_samples)} files (25 segments across {len(speakers)} voices) for Arabic A/B check.")
    ...
    base_results_dir = "eval_base_ar_samples"
    ...
    for ref_name in tqdm(speakers, desc="Speakers"):
        ...
        if not os.path.exists(ref_path): ref_path = f"blind_test/audio/ar/{ref_name}.wav"
    ...
    print("\n" + "="*70)
    print(f"🏆 REPRESENTATIVE A/B TEST: ARABIC (AR) - {len(eval_samples)} samples")
    print("="*70)
    ...
    if lora_cer < base_cer and lora_sim > base_sim:
        print("\n✅ STATUS: CLEAR UPGRADE. Step 400 AR is numerically superior in all metrics.")
    else:
        print("\n⚠️ STATUS: MIXED. Results show quality trade-offs for Arabic.")
